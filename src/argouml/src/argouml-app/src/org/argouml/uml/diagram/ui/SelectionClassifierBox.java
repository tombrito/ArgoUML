 ...
 */
public abstract class SelectionClassifierBox extends SelectionNodeClarifiers2 {
	...
	private class Button {
		...
		public Button(int handle, Icon icon, Fig fig, Object metaType) {
			this.handle = handle;
			this.fig = fig;
			this.icon = icon;
	...
	public void hitHandle(Rectangle cursor, Handle h) {
		...
		if (h.index == -1) {
			if (hitBelow(cx + cw - (addIcon.getIconWidth() / 2), cy + 25, addIcon.getIconWidth() + 5,
					addIcon.getIconHeight() + 3, cursor)) {
				h.index = UPPER_RIGHT;
			} else if (hitAbove(cx + cw - (addIcon.getIconWidth() / 2), cy + ch, addIcon.getIconWidth() + 3,
					addIcon.getIconHeight() + 5, cursor)) {
				h.index = LOWER_RIGHT;
			}
		}
	}
