...
/**
 * A Fig that can represent a UML2 vertex. The current implementation is limited
 * to just handling States. Other vertices are currently handled by different
 * Figs but they will most likely all be handled here in future.
 */
public class FigVertex extends FigNodeModelElement {
	 ...
	 * 
	 * @param owner
	 *            the vertex that own this dagram element
	 * @param bounds
	 *            the bounds of the diagram element
	 * @param settings
	 *            the display settings for this diagram element
	 */
	...
	public void setEnclosingFig(Fig encloser) {
		...
		if (encloser != null) {
			...
			if (regions.isEmpty()) {
				region = Model.getUmlFactory().buildNode(Model.getMetaTypes().getRegion(), stateMachine);
			} else {
				region = regions.get(0);
			}
		}
		if (region != null && Model.getFacade().getContainer(getOwner()) != region) {
	...
	protected void modelChanged(PropertyChangeEvent mee) {
		...
		if (mee instanceof AddAssociationEvent) {
			...
			rg.setBounds(regionCompartment.getX(), regionCompartment.getY(), rg.getMinimumSize().width,
					rg.getMinimumSize().height);
	...
	public void renderingChanged() {
		...
		if (notationProviderBody != null) {
			bodyText.setText(notationProviderBody.toString(getOwner(), getNotationSettings()));
		}
		calcBounds();
		setBounds(getBounds());
	}
	...
	public Dimension getMinimumSize() {
		...
		int h = getTopMargin() + nameSize.height + getBottomMargin();
		...
		int w = getLeftMargin() + Math.max(nameSize.width, bodySize.width) + getRightMargin();
	...
	protected void setStandardBounds(int x, int y, int w, int h) {
		...
		getNameFig().setBounds(x + getLeftMargin(), y + getTopMargin(), w - getLeftMargin() - getRightMargin(),
				nameSize.height);

		if (getBodyText().getText().length() > 0) {
			getBodyText().setBounds(x + getLeftMargin(), y + getTopMargin() + nameSize.height, bodySize.width,
					bodySize.height);
		} else {
			getBodyText().setBounds(x + getLeftMargin(), y + getTopMargin() + nameSize.height, bodySize.width, 1);
		}

		regionCompartment.setBounds(x + getLeftMargin(), getBodyText().getY() + getBodyText().getHeight(),
				w - getLeftMargin() - getRightMargin(),
				h - getTopMargin() - getBottomMargin() - getNameFig().getHeight() - getBodyText().getHeight());
	...
	public Vector getPopUpActions(MouseEvent me) {
		...
		if (TargetManager.getInstance().getTargets().size() == 1) {
			popUpActions.add(popUpActions.size() - getPopupAddOffset(),
					new ActionCreateContainedModelElement(Model.getMetaTypes().getRegion(), getOwner()));
		}
		return popUpActions;
	}
		...
		public void paint(Graphics g) {
			...
			for (Iterator it = getFigs().iterator(); it.hasNext();) {
				...
				if (it.hasNext()) {
					...
					drawDashedLine(g, 1, fig.getX(), fig.getY() + fig.getHeight(), fig.getX() + fig.getWidth(),
							fig.getY() + fig.getHeight(), 0, new float[] { 5.0f, 5.0f }, 10);
				}
			}
