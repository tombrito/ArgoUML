...
/**
 * A factory for creating context menu actions to place related nodes and edges
 * of an existing FigNode.
 *
 * @author Bob Tarling
 */
public class RelationshipActionFactory implements ContextActionFactory {
							...
							if (connection != associationEnd) {
								if (Model.getFacade().isNavigable(associationEnd)) {
									...
									al.add(new AddAssociationAction(
											"Add association from " + Model.getFacade().getName(oppositeClass), diagram,
											element, association, oppositeClass));
								}
							}
						}
					}
						...
						for (Object connection : connections) {
							if (connection != associationEnd) {
								if (!Model.getFacade().isNavigable(associationEnd)) {
									Object oppositeClass = Model.getFacade().getClassifier(connection);
									al.add(new AddAssociationAction(
											"Add association to " + Model.getFacade().getName(oppositeClass), diagram,
											element, association, oppositeClass));
								}
							}
						}
			...
			for (Object dependency : dependenciesTo) {
				...
				if (diagram.presentationFor(dependency) == null) {
					...
					for (Object supplier : suppliers) {
						al.add(new AddUsageAction("Add dependancy to " + Model.getFacade().getName(supplier), diagram,
								element, dependency, supplier));
					}
				}
			}
		}
			...
			for (Object dependency : dependenciesFrom) {
				...
				if (diagram.presentationFor(dependency) == null) {
					...
					for (Object client : clients) {
						al.add(new AddUsageAction("Add dependancy from " + Model.getFacade().getName(client), diagram,
								element, dependency, client));
					}
				}
			}
		}
	...
	private ActionList getGeneralizationToActions(final Object element, ArgoDiagram diagram) {
		...
		for (Object generalization : generalizations) {
			...
			if (diagram.presentationFor(generalization) == null) {
				...
				al.add(new AddGeneralAction("Add Generalization to " + Model.getFacade().getName(general), diagram,
	...
	private ActionList getGeneralizationFromActions(final Object element, ArgoDiagram diagram) {
		...
		for (Object specialization : specializations) {
			...
			if (diagram.presentationFor(specialization) == null) {
				...
				al.add(new AddSpecialAction("Add Specialization from " + Model.getFacade().getName(specific), diagram,
						element, specialization, specific));
			}
		}
		return al;
	}
	...
	private class AddAssociationAction extends AbstractAction {
		...
		public AddAssociationAction(final String name, final ArgoDiagram diagram, final Object element,
				final Object association, final Object oppositeClass) {
			super(name);
		...
		public void actionPerformed(ActionEvent arg0) {
			if (diagram.presentationFor(oppositeClass) == null) {
				...
				addToDiagram(diagram, oppositeClass, new Rectangle(rect.x + rect.width + 100, rect.y, 0, 0));
			}
	...
	private class AddUsageAction extends AbstractAction {
		...
		public AddUsageAction(final String name, final ArgoDiagram diagram, final Object element, final Object usage,
				final Object oppositeClass) {
			super(name);
			this.element = element;
			this.usage = usage;
	...
	private class AddGeneralAction extends AbstractAction {
		...
		public AddGeneralAction(final String name, final ArgoDiagram diagram, final Object element,
				final Object generalization, final Object generalClass) {
			super(name);
			this.element = element;
		...
		public void actionPerformed(ActionEvent arg0) {
			if (diagram.presentationFor(generalClass) == null) {
				Rectangle rect = diagram.presentationFor(element).getBounds();
				addToDiagram(diagram, generalClass, new Rectangle(rect.x, rect.y - (rect.height + 100), 0, 0));
			}
	...
	private class AddSpecialAction extends AbstractAction {
		...
		public AddSpecialAction(final String name, final ArgoDiagram diagram, final Object element,
				final Object generalization, final Object specialClass) {
			super(name);
			this.element = element;
			this.generalization = generalization;
			this.diagram = diagram;
		...
		public void actionPerformed(ActionEvent arg0) {
			if (diagram.presentationFor(specialClass) == null) {
				Rectangle rect = diagram.presentationFor(element).getBounds();
				addToDiagram(diagram, specialClass, new Rectangle(rect.x, rect.y + rect.height + 100, 0, 0));
			}
			addToDiagram(diagram, generalization, new Rectangle());
		}
	}
