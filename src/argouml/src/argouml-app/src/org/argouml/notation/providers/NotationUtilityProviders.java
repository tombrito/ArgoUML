...
class NotationUtilityProviders {
	...
	static void addListenersForTransition(NotationProvider np, Object transition) {
		np.addElementListener(transition, new String[] { "guard", "trigger", "effect" });
	...
	private static void addListenersForEvent(NotationProvider np, Object event) {
		if (event != null) {
			if (Model.getFacade().isAEvent(event)) {
				np.addElementListener(event, new String[] { "parameter", "name" });
			}
			if (Model.getFacade().isATimeEvent(event)) {
				np.addElementListener(event, new String[] { "when" });
			}
			if (Model.getFacade().isAChangeEvent(event)) {
				np.addElementListener(event, new String[] { "changeExpression" });
			}
	...
	static void addListenersForAction(NotationProvider np, Object action) {
		if (action != null) {
			np.addElementListener(action, new String[] { "script", "actualArgument", "action" });
			/* And the arguments: */
